...
export default function Dashboard({ onNavigatePilot, refreshKey }: DashboardProps) {
    ...
    if (!data) return <div className="flex h-full items-center justify-center bg-slate-50 text-slate-400 text-xs font-bold uppercase tracking-widest animate-pulse">Cargando...</div>;
                ...
                <div className="px-8 mt-6 flex justify-between items-start w-full relative z-40 pointer-events-none">
                    ...
                    <div className="pointer-events-auto">
                        <div className="flex items-center gap-4 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm">
                            <div className="flex flex-col gap-0 text-left">
                                <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-0.5">Año</label>
                                <div className="relative group/select">
                                    <select
                                        value={filterYear}
                                        onChange={(e) => setFilterYear(e.target.value === 'Todos' ? 'Todos' : parseInt(e.target.value))}
                                        className="bg-transparent border-none text-sm font-bold text-slate-700 focus:ring-0 outline-none cursor-pointer p-0 pr-6 leading-none appearance-none transition-colors hover:text-slate-900"
                                    >
                    ...
                    <div className="pointer-events-auto">
                        <button
                            ...
                            className="bg-slate-900 hover:bg-slate-800 text-white w-12 h-12 rounded-xl flex items-center justify-center transition-all shadow-lg hover:shadow-slate-900/20 hover:scale-105 active:scale-95 group"
                ...
                <div className="flex-1 overflow-y-auto px-8 pb-8 w-full no-scrollbar z-10 pt-4">
                    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-slate-50/80 border-b border-slate-200 text-slate-500 text-[10px] font-bold uppercase tracking-widest sticky top-0 z-10 backdrop-blur-md">
                                <tr>
                            ...
                            <tbody className="divide-y divide-slate-100 text-sm">
                                {data.transacciones.map((tx: any) => {
                                    ...
                                    const renderDates = () => {
                                        if (tipo === 'reciproco') {
                                            ...
                                            return (
                                                <div className="flex flex-col">
                                                    {renderRangeList(rangesIda, "Ida", "text-purple-500")}
                                                    {renderRangeList(rangesVuelta, "Vuelta", "text-emerald-500")}
                                                </div>
                                            );
                                        }
                                        ...
                                        if (ranges.length === 0) return <span className="text-slate-300">-</span>;
                                    ...
                                    return (
                                        <tr key={tx.id} className={clsx("hover:bg-slate-50 transition-colors group", (tx.estado === 'anulado' || hasInactivePilot) && "bg-slate-50/50 opacity-60 grayscale")}>
                                            <td className="px-6 py-4 align-top">
                                                <div className="flex flex-col items-center gap-1">
                                                    <span className={clsx(
                                                        "font-bold px-2 py-0.5 rounded text-xs ring-1 ring-inset",
                                                        tipo === 'reciproco' ? "bg-purple-50 text-purple-700 ring-purple-100" :
                                                            tipo === 'condonacion' ? "bg-amber-50 text-amber-700 ring-amber-100" : "bg-slate-100 text-slate-600 ring-slate-200"
                                                    )}>
                                            ...
                                            <td className="px-6 py-4 align-top">
                                                <div className="flex flex-col gap-1.5">
                                                    {renderDates()}
                                                    {tx.resumen_dias > 0 && (
                                                        <span className="text-[9px] bg-slate-900 text-white px-2 py-0.5 rounded font-bold uppercase tracking-wider w-fit">
                                            ...
                                            <td className="px-6 py-4 align-top">
                                                <div className="flex flex-col gap-3">
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        <button onClick={() => onNavigatePilot(tx.acreedor_id)} className="font-bold text-slate-700 min-w-32 hover:text-slate-900 transition-colors text-left text-sm">
                                                            {tx.acreedor_nombre}
                                                        </button>

                                                        {tipo === 'reciproco' ? (
                                                            <div className="w-6 h-6 rounded-full bg-purple-50 text-purple-600 border border-purple-100 flex items-center justify-center flex-shrink-0">
                                                            ...
                                                            <div className="w-6 h-6 rounded-full bg-amber-50 text-amber-600 border border-amber-100 flex items-center justify-center flex-shrink-0">
                                                        ...
                                                        <button onClick={() => onNavigatePilot(tx.deudor_id)} className="font-bold text-slate-700 min-w-32 hover:text-slate-900 transition-colors text-right text-sm">
                                                            {tx.deudor_nombre}
                                                        </button>
                                                    </div>
                                                    ...
                                                    {tipo === 'unilateral' && datos.realizado_por_id && (
                                                        <div className="bg-slate-50 p-1.5 rounded border border-slate-200 border-l-2 border-l-amber-500 flex items-center gap-2 max-w-fit">
                                                            <span className="text-[9px] text-slate-500 uppercase font-bold tracking-wider">Triangulación</span>
                                                            <span className="text-[10px] text-slate-700 font-bold">{practicos.find(p => p.id === datos.realizado_por_id)?.nombre || datos.realizado_por_id}</span>
                                                        </div>
                                                    )}
                                                    ...
                                                    {tipo === 'reciproco' && (
                                                        <div className="flex items-center gap-3 text-[10px] text-slate-500">
                                                            <div className="flex items-center gap-1.5 bg-slate-50 px-2 py-0.5 rounded border border-slate-100">
                                                                <span className="uppercase font-bold tracking-wider text-slate-400">Ida</span>
                                                                <span className="font-semibold">{datos.ida.dias} días</span>
                                                                {datos.ida.realizado_por_id && <span className="text-purple-600 font-bold ml-1">TR</span>}
                                                            </div>
                                                            <div className="flex items-center gap-1.5 bg-slate-50 px-2 py-0.5 rounded border border-slate-100">
                                                                <span className="uppercase font-bold tracking-wider text-slate-400">Vuelta</span>
                                                                <span className="font-semibold">{datos.vuelta.dias} días</span>
                                                                {datos.vuelta.realizado_por_id && <span className="text-emerald-600 font-bold ml-1">TR</span>}
                                                            </div>
                                                        </div>
                                                    )}

                                                    {tx.observacion && (
                                                        <div className="mt-2 flex items-start gap-2 bg-slate-50 p-2 rounded-lg border border-slate-100">
                                                            ...
                                                            <p className="text-xs text-slate-600 font-medium leading-snug">{tx.observacion}</p>
                                                        </div>
                                                    )}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-right align-top">
                                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button onClick={() => handleEdit(tx)} className="text-slate-400 hover:text-slate-700 p-1.5 hover:bg-slate-100 rounded">
                                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                                    </button>
                                                    <button onClick={() => { setDeletingId(tx.id); setShowDeleteModal(true); }} className="text-slate-400 hover:text-rose-600 p-1.5 hover:bg-rose-50 rounded">
                        ...
                        {data.transacciones.length === 0 && (
                            <div className="py-24 text-center">
                                <p className="text-slate-300 uppercase tracking-widest text-[10px] font-bold">No hay registros para este período</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200 p-6">
                        <div className="text-center">
                            <h3 className="text-lg font-bold text-slate-800 uppercase tracking-wide mb-2">Eliminar Registro</h3>
                        ...
                        <div className="space-y-3">
                            <button
                                ...
                                className="w-full border border-slate-200 hover:border-slate-800 hover:bg-slate-50 text-slate-700 p-3 rounded-lg flex flex-col items-center transition-all group"
                            >
                                <span className="font-bold uppercase text-xs tracking-wider">Opción 1: Liberar Nro</span>
                                <span className="text-[10px] text-slate-400 mt-1">Crea un hueco en el libro</span>
                            </button>

                            <button
                                onClick={confirmDeleteShift}
                                className="w-full bg-slate-900 hover:bg-slate-800 text-white p-3 rounded-lg flex flex-col items-center transition-all shadow-lg"
                            >
                                <span className="font-bold uppercase text-xs tracking-wider text-slate-50">Opción 2: Eliminar y Reordenar</span>
                                <span className="text-[10px] text-slate-400 mt-1">Renumera los registros siguientes</span>
                            </button>
